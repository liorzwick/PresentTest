...
# ערכי המוח מהבקטסט המנצח מוטמעים כאן ישירות
BRAIN = {
    "max_base_depth": 0.35, 
...
def load_tickers():
    """טוען מניות מ-mystock.csv או מרשימת גיבוי"""
    ...
    print("⚠️ משתמש ברשימת גיבוי (AAPL, NVDA, TSLA, וכו')...")
    return ['AAPL', 'MSFT', 'NVDA', 'TSLA', 'AMZN', 'META', 'GOOGL', 'PLTR', 'AMD', 'NFLX']

# ==========================================
# 2. פונקציות עזר טכניות
# ==========================================
...
def get_ascending_triangle_signal(hist):
    ...
    v1_price = float(np.min(post_peak1_lows)) if 'post_peak1_lows' in locals() else float(np.min(post_p1_lows))
...
def run_standalone_test():
    ...
    if not market_ok:
        print("🔴 אזהרה: מדד ה-SPY מתחת לממוצע 200. השוק בסיכון גבוה.")
    ...
    for ticker in tickers:
        ...
        try:
            ...
            # פילטרים בסיסיים (מחיר, נזילות, מגמה)
            ...
            if is_breakout or is_near:
                ...
                if is_breakout:
                    # פריצה דורשת ווליום
                    if vol_ratio < BRAIN['breakout_volume_ratio']: continue
    ...
    if alerts:
        ...
        for a in alerts:
            ...
            print(f"📐 תבנית: {a['type']} | מרחק מפיבוט: {a['dist']:.1f}%")
            print(f"💰 מחיר: ${a['close']:.2f} (פיבוט: ${a['pivot']:.2f})")
            print(f"🛡️ סטופ מומלץ: ${a['stop']:.2f} ({a['risk']:.1f}%)")
            print(f"📊 ווליום: {a['vol']:.1f}x")
            print("-" * 30)
    else:
