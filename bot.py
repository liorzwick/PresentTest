...
def load_brain():
    brain = {
        ...
        # שדרוג: פרמטרים חדשים
        "min_vcp_contraction": 0.85,   # יחס ATR מאוחר/מוקדם — חייב להיות מתחת לזה
        "min_pole_vol_ratio": 0.85,    # נפח דגל חייב להיות מתחת ל-85% מנפח התורן
        "rs_periods": [63, 126, 189],  # תקופות RS משוקלל
...
def save_to_smart_memory(ticker, price, stop_loss, risk_pct, vol_ratio, pivot, close_strength,
                          rs_weighted, tightness, pattern_type, status, setup_score,
                          dry_up_ratio, touches, contraction_ratio):
    ...
    new_record = pd.DataFrame([{
        ...
        "Close_Strength": round(float(close_strength), 2), "RS_Weighted": round(float(rs_weighted), 4),
...
def add_indicators(df):
    ...
    # שדרוג: שלוש תקופות ROC למדד RS משוקלל
...
# שדרוג: RS משוקלל על 3 תקופות במקום ROC_65 יחיד
...
def get_dtw_templates():
    ...
    # דגל שורי — חלון מינימום 30 ימים (תורן אמיתי)
    ...
    templates["🚩 דגל שורי"] = {
        ...
        "windows": [30, 45, 60], "threshold": 0.12, "min_corr": 0.88, "comp": 0
    }
    ...
    # ספל וידית — sin() לידית טבעית יותר
...
def check_all_dtw_patterns(closes, volumes, atr_values):
    """
    שדרוג מרכזי:
    1. Composite signal = Close (75%) + Volume (25%) — DTW על שניהם
    2. פיבוט על percentile 95 במקום max (חסין לספייקים)
    3. Volume Dry-Up בתוך הבסיס (לדגל בלבד)
    4. VCP Contraction — ATR מתכווץ לקראת הפריצה
    5. ספירת נגיעות אמיתיות ברמת הפיבוט
    """
    ...
    for name, config in templates.items():
        for window in config["windows"]:
            ...
            # מנעול: מניעת תנודתיות רעשית (שיני מסור)
            ...
            # קורלציה על המחיר בלבד (composite לא מתאים לקורלציה לינארית)
            corr = np.corrcoef(norm_price, norm_template_resized)[0, 1]
            if pd.isna(corr) or corr < config["min_corr"]:
                continue

            # מנעול: היגיון אנושי במחירי אמת
            w = window
            if "דגל" in name:
                if current_closes[-1] <= current_closes[0] * 1.05:
                    continue
                # שדרוג: Volume Dry-Up — נפח בחלק הדגל חייב לרדת מהתורן
            ...
            # שדרוג: VCP Contraction — ATR מאוחר חייב להיות קטן מה-ATR המוקדם
            ...
            # DTW על composite signal
            distance, _ = fastdtw(norm_composite, norm_template_resized, dist=lambda x, y: abs(x - y))
            avg_distance = distance / window

            if avg_distance < config["threshold"] and avg_distance < best_score:
                best_score = avg_distance

                # שדרוג: פיבוט על percentile 95 — חסין לספייקים
                if "דגל" in name:
                ...
                # שדרוג: ספירת נגיעות אמיתיות ברמת הפיבוט (±2%)
                touches = int(sum(
                    1 for p in current_closes
                    if pivot > 0 and abs(p - pivot) / pivot < 0.02
                ))
                touches = max(touches, 2)  # מינימום 2 כדי לא לשבור ספסל קיים
                ...
                best_pattern = {
                    ...
                    "tightness": tightness,
                    "base_depth": 0.20,
                    "dry_up_ratio": round((flag_vol / pole_vol) if "דגל" in name and pole_vol > 0 else 1.0, 2),
                    "touches": touches,
                    "base_length": window,
...
def calc_setup_score(alert):
    ...
    # DTW + קורלציה — קורלציה במשקל כפול (הפקטור המרכזי)
    ...
    # שדרוג: בונוס VCP — ככל שה-ATR מתכווץ יותר, הציון גבוה יותר
    vcp_bonus = max(0, (BRAIN["min_vcp_contraction"] - alert.get("contraction_ratio", 1.0)) * 30)

    # שדרוג: בונוס נגיעות — כל נגיעה מעל 2 שווה 3 נקודות
...
def scan_market():
    ...
    for ticker in tickers:
        ...
        try:
            ...
            # שדרוג: מגמה מסודרת SMA50 > SMA150 > SMA200 (Stage 2 Weinstein)
            sma50  = float(today["SMA_50"])
            sma150 = float(today["SMA_150"])
            sma200 = float(today["SMA_200"])
            weekly_trend_ok = (sma50 > sma150 > sma200)
            is_below_150    = close < sma150

            if not weekly_trend_ok and not is_below_150:
                # מגמה לא בשלה ואין סיבה לנסות — דלג
                pass
            stats["pass_trend"] += 1
            ...
            # חלון סינון מצומצם — ספסל עמוק מ-10% לא עובר
            ...
            alert_data = {
                ...
                "rs_weighted": stock_rs, "close_strength": close_strength, "status": status,
    ...
    if all_potentials:
        ...
        for idx, s in enumerate(sorted_all, 1):
            ...
            print(f"{idx}. {s['ticker']:<5} | תבנית: {clean_type:<15} | ציון: {s['setup_score']:<5.1f} | "
    ...
    # מיון לפי קורלציה ראשונה, ציון משני
    ...
    msg  = "🎯 <b>סריקת ראייה ממוחשבת מרובת זמנים (DTW v3)!</b>\n"
    msg += f"<i>(מציג עד {TOP_RESULTS} מניות — ממוינות לפי קורלציה)</i>\n\n"
        ...
        for a in stocks:
            ...
            if a["status"] != "🪑 ספסל":
                save_to_smart_memory(
                    ...
                    a["vol_ratio"], a["pivot"], a["close_strength"], a["rs_weighted"],
                    a["tightness"], a["type"], a["status"], a["setup_score"],
                    a["dry_up_ratio"], a["touches"], a["contraction_ratio"]
                )
