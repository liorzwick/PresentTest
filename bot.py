...
def load_brain():
    brain = {
        ...
        "min_rs_65": 0.02,
        # max_dist_from_52w_high ×ž×•×’×“×¨ ×›×¢×ª ×‘× ×¤×¨×“ ×œ×›×œ ×ž×¦×‘:
        # ×ž× ×™×•×ª ×ž×¢×œ 150 â†’ 0.25, ×ž× ×™×•×ª ×ž×ª×—×ª ×œ-150 â†’ 0.45
    ...
    try:
        ...
        print(f"âš ï¸ ××–×”×¨×”: ×œ× × ×™×ª×Ÿ ×œ×˜×¢×•×Ÿ ××ª brain.json, ×ž×©×ª×ž×© ×‘×‘×¨×™×¨×•×ª ×ž×—×“×œ.")
    return brain
...
def send_telegram(message):
    ...
    if not TELEGRAM_TOKEN or not TELEGRAM_CHAT_ID:
        print("âš ï¸ [×ž×¦×‘ ×¡×™×ž×•×œ×¦×™×” - ×”×˜×•×§×Ÿ ××• ×”-ID ×—×¡×¨×™× ×‘-GitHub Secrets]")
        return
    ...
    try:
        ...
        if response.status_code != 200:
            print(f"âŒ ×˜×œ×’×¨× ×—×¡× ××ª ×”×”×•×“×¢×” ×œ×§×‘×•×¦×”! ×”×ª×©×•×‘×” ×©×œ ×˜×œ×’×¨×: {response.text}")
        else:
            print("âœ… ×”×”×•×“×¢×” × ×©×œ×—×” ×‘×”×¦×œ×—×” ×œ×§×‘×•×¦×ª ×”×˜×œ×’×¨×!")
    except Exception as e:
        print(f"âŒ ×©×’×™××ª ×ª×§×©×•×¨×ª ×¢× ×˜×œ×’×¨×: {e}")
...
def save_to_smart_memory(ticker, price, stop_loss, risk_pct, vol_ratio, pivot,
                         close_strength, rs_65, tightness, pattern_type, status):
    memory_file = "smart_memory.csv"
    now = datetime.now().strftime("%Y-%m-%d")
    new_record = pd.DataFrame([{
        "Date": now, "Ticker": ticker, "Price": round(price, 2),
        "Pivot": round(pivot, 2), "Stop_Loss": round(stop_loss, 2),
        "Risk_Pct": round(risk_pct, 2), "Volume_Ratio": round(vol_ratio, 2),
        "Close_Strength": round(close_strength, 2), "RS_65": round(rs_65, 4),
...
def load_tickers():
    if os.path.exists(CUSTOM_TICKERS_FILE):
        try:
            ...
            if col_name:
                tickers = (
                    df[col_name].dropna().astype(str).str.strip().str.upper().tolist()
                )
                tickers = [
                    t.replace('.', '-') for t in tickers
                    if t.isalpha() or '-' in t or '.' in t
                ]
                print(f"âœ… × ×˜×¢× ×• {len(set(tickers))} ×ž× ×™×•×ª ×ž×ª×•×š {CUSTOM_TICKERS_FILE}")
...
# [×ª×™×§×•×Ÿ 1] ×‘×“×™×§×ª Market Cap ××ž×™×ª×™×ª ×“×¨×š yfinance
def check_market_cap(ticker):
    """
    ×ž×—×–×™×¨ ××ª ×”-market cap ×©×œ ×”×ž× ×™×”.
    ×ž×©×ª×ž×© ×‘-fast_info ×§×•×“× (×ž×”×™×¨), ×•×× × ×›×©×œ, ×¢×•×‘×¨ ×œ-info (××˜×™).
    ×ž×—×–×™×¨ None ×× ×œ× × ×™×ª×Ÿ ×œ×ž×©×•×š.
    """
    try:
        t = yf.Ticker(ticker)
        # fast_info ×ž×”×™×¨ ×™×•×ª×¨, ××š ×œ× ×ª×ž×™×“ ×§×™×™×
...
# [×ª×™×§×•×Ÿ 2] ×ž×•×¦× swing highs ×ž×§×•×ž×™×™× ×‘×ž×§×•× argmax ×’×œ×•×‘×œ×™
def find_swing_highs(arr, window=5):
    """
    ×ž×—×–×™×¨ ×¨×©×™×ž×” ×©×œ (index, value) ×©×œ ×©×™××™× ×ž×§×•×ž×™×™×.
    ×©×™× ×ž×§×•×ž×™ = ×”× ×§×•×“×” ×”×’×‘×•×”×” ×‘×™×•×ª×¨ ×‘×—×œ×•×Ÿ ×©×œ window ×™×ž×™× ×œ×›×œ ×¦×“.
    """
    peaks = []
...
# [×ª×™×§×•×Ÿ 3] ×ž×•×¦× swing lows ×ž×§×•×ž×™×™×
def find_swing_lows(arr, window=5):
    """
    ×ž×—×–×™×¨ ×¨×©×™×ž×” ×©×œ (index, value) ×©×œ ×©×¤×œ×™× ×ž×§×•×ž×™×™×.
    """
    troughs = []
    for i in range(window, len(arr) - window):
...
def market_filter_ok(spy_df):
    ...
    if pd.isna(today["SMA_200"]) or pd.isna(sma200_old):
        return False
    return (
        (today["Close"] > today["SMA_50"] > today["SMA_150"] > today["SMA_200"])
        and (today["SMA_200"] > sma200_old)
    )

# ==========================================
# ×¤×•× ×§×¦×™×•×ª ×ª×‘× ×™×ª â€” ×ž×©×ª×ž×©×•×ª ×‘-Swing Highs/Lows
# ==========================================

def get_ascending_triangle_signal(hist):
    """
    [×ª×™×§×•×Ÿ ×ž×¨×›×–×™] ×”×•×—×œ×¤×• argmax/argmin ×’×œ×•×‘×œ×™×™× ×‘-swing highs/lows ×ž×§×•×ž×™×™×.
    ×ž×—×¤×© VCP ×§×œ××¡×™: ×©×™× â†’ ×¢×ž×§ â†’ ×©×™× ×©× ×™ ×§×¨×•×‘ â†’ ×¢×ž×§ ×’×‘×•×” ×™×•×ª×¨ â†’ ×›×™×•×•×¥.
    """
    ...
    if tightness > BRAIN['max_tightness_depth']:
        return None
...
def get_cup_handle_signal(hist):
    """
    [×ª×™×§×•×Ÿ ×ž×¨×›×–×™] ×”×•×—×œ×£ argmax ×’×œ×•×‘×œ×™ ×‘-swing highs.
    ×ž×—×¤×© rim ×ž×”×©×™××™× ×”×ž×§×•×ž×™×™×, ×•×œ× ×ž×”×ž×§×¡×™×ž×•× ×”×’×œ×•×‘×œ×™.
    """
    ...
    # --- ×©×œ×‘ 1: ×ž×¦× rim ×ž-swing highs (×¤×¨×˜ ×œ-21 ×™×ž×™× ××—×¨×•× ×™×)
    swing_highs = find_swing_highs(highs[:-21], window=5)
    if not swing_highs:
        return None
    ...
    if len(cup_section) < 21:
        return None

    cup_low = float(np.min(cup_section))
    depth   = (rim_price - cup_low) / rim_price

    if not (BRAIN['min_cup_depth'] <= depth <= BRAIN['max_cup_depth']):
        return None

    # --- ×©×œ×‘ 3: ×™×“×™×ª ×‘×¦×“ ×™×ž×™×Ÿ â€” ×‘×—×œ×•×Ÿ ×©×œ 21 ×™×ž×™× ××—×¨×•× ×™×
    cup_to_now = recent.iloc[rim_idx:]
    if len(cup_to_now) < 21:
        return None

    handle_low = float(cup_to_now["Low"].min())

    # ×”×™×“×™×ª ×œ× ×™×›×•×œ×” ×œ×¨×“×ª ×ž×ª×—×ª ×œ-35% ×ž×¢×•×ž×§ ×”×›×•×¡
    if handle_low < (cup_low + 0.35 * (rim_price - cup_low)):
        return None
...
def get_multi_touch_vcp_signal(hist):
    """
    Flat Base: ×ž×—×¤×© ×œ×¤×—×•×ª 3 × ×’×™×¢×•×ª ×‘××•×ª×” ×¨×ž×ª ×”×ª× ×’×“×•×ª (×¡×˜×™×™×” ×¢×“ 4%).
    ×”×¤×•× ×§×¦×™×” ×”×–×• ×›×‘×¨ ×”×©×ª×ž×©×” ×‘×’×™×©×ª swing highs â€” × ×©××¨×” ×›×ž×¢×˜ ×œ×œ× ×©×™× ×•×™
    ×¢× ×©×™×¤×•×¨ ×§×œ ×‘-window ×•×‘-tightness.
    """
    recent = hist.tail(250)
    if len(recent) < 60:
    ...
    for p_idx, p_val in peaks:
        group = [
            idx for idx, val in peaks
            if abs(val - p_val) / max(p_val, 1e-9) <= 0.04
        ]
        if len(group) > len(best_group) or (
            len(group) == len(best_group) and p_val > best_pivot
        ):
            best_group = group
    ...
    if best_group[-1] - best_group[0] < 30:
        return None
    ...
    if depth > BRAIN['max_base_depth'] or depth < 0.05:
        return None
...
def scan_market():
    ...
    if spy.empty:
        ...
        if not market_filter_ok(spy):
            market_warning = (
                "âš ï¸ <b>×©×™× ×œ×‘: ×”×©×•×§ ×”×›×œ×œ×™ ×—×œ×© (SPY ×ž×ª×—×ª ×œ×ž×ž×•×¦×¢×™×).</b> "
                "×”×¡×¨×™×§×” ×ž×ž×©×™×›×” ×œ×‘×§×©×ª×š, ××š ×”×¡×™×›×•×Ÿ ×œ×¤×¨×™×¦×•×ª ×©×•×•× ×’×‘×•×”.\n\n"
            )

    all_potentials = []

    # ×ž×˜×ž×•×Ÿ market caps ×›×“×™ ×œ× ×œ×”×¨×™×¥ API call ×›×¤×•×œ
    market_cap_cache = {}

    for ticker in tickers:
        print(f"×¡×•×¨×§ ××ª {ticker}...", end="\r")
        try:
            ...
            # --- ×¤×™×œ×˜×¨×™× ×‘×¡×™×¡×™×™× ---
            if any(pd.isna(today[c]) for c in ["SMA_50", "SMA_150", "SMA_200", "ATR_14"]):
                continue
            if float(today["Close"]) < MIN_PRICE:
                continue
            if float(today["DollarVol_50"]) < MIN_DOLLAR_VOL_50:
                continue

            # [×ª×™×§×•×Ÿ: ×‘×“×™×§×ª Market Cap ××ž×™×ª×™×ª]
            ...
            if mc is not None and mc < MIN_MARKET_CAP:
                print(f"  â†³ {ticker} × ×¤×¡×œ: market cap ${mc/1e9:.1f}B < ×ž×™× ×™×ž×•×")
                continue

            # --- ×ª× ××™ ×ž×’×ž×” ---
            if not (float(today["Close"]) > float(today["SMA_50"])):
                continue
            ...
            if high_252 is not None and high_252 > 0:
                ...
                max_dist = (
                    BRAIN['max_dist_from_52w_high_below_150'] if is_below_150
            ...
            # --- ×–×™×”×•×™ ×ª×‘× ×™×ª (Swing Highs) ---
            pattern = (
                get_ascending_triangle_signal(past_data)
                or get_cup_handle_signal(past_data)
                or get_multi_touch_vcp_signal(past_data)
            )
            if not pattern:
                continue

            if is_below_150 and pattern["tightness"] > 0.05:
                continue
            ...
            if is_breakout or is_near_breakout:
                ...
                if is_breakout:
                    ...
                    if (close_strength < req_close
                            or vol_ratio < req_vol
                            or gap_from_pivot > BRAIN['max_gap_above_pivot']):
                        continue
                    status = "ðŸ”¥ ×¤×¨×™×¦×” ×¤×¢×™×œ×”!"
                else:
                    status = "ðŸ‘€ ×ž×ª×‘×©×œ×ª (Watchlist)"
                ...
                if float(today["Close"]) <= pivot * (1 + BRAIN['max_entry_extension']):
                    ...
                    alert_data = {
                        ...
                        "market_cap":    mc_str,
                        "dist_52w":      dist_52w * 100 if high_252 else None
                    }
    ...
    final_bo = [s for s in final_selection if "×¤×¨×™×¦×” ×¤×¢×™×œ×”" in s["status"]]
    final_wl = [s for s in final_selection if "×ž×ª×‘×©×œ×ª" in s["status"]]

    total_sent = len(final_bo) + len(final_wl)

    if total_sent > 0:
        print(f"ðŸ”¥ ×”×¡×¨×™×§×” ×”×¡×ª×™×™×ž×”! ×ž×¦×™×’ {total_sent} ×ž× ×™×•×ª "
              f"(×ž×ª×•×›×Ÿ {below_150_count} ×ž×ª×—×ª ×œ×ž×ž×•×¦×¢ 150) ×‘×˜×œ×’×¨×.")

        msg  = "ðŸŽ¯ <b>×¡×¨×™×§×ª VCP ×™×•×ž×™×ª ×”×¡×ª×™×™×ž×”!</b>\n"
        if market_warning:
            msg += f"{market_warning}\n"

        msg += (
            f"<i>(×ž×¦×™×’ {total_sent} ×ž× ×™×•×ª ×§×¨×•×‘×•×ª ×œ×¤×™×‘×•×˜. "
            f"×ž×§×¡×™×ž×•× 3 ×ž×ª×—×ª ×œ×ž×ž×•×¦×¢ 150)</i>\n\n"
        )

        def format_stock_block(a, icon):
            tv_link      = f"https://il.tradingview.com/chart/?symbol={a['ticker']}"
            warning_150  = " âš ï¸ (×ž×ª×—×ª ×œ-150)" if a["is_below_150"] else ""
            dist_52w_str = (
                f" | ðŸ“‰ <b>×ž-52W High:</b> {a['dist_52w']:.1f}%"
                if a["dist_52w"] is not None else ""
            )
            line  = f"{icon} <b>{a['ticker']}</b> | ×ª×‘× ×™×ª: {a['type']}{warning_150}\n"
            line += f"ðŸ’¼ <b>×©×•×•×™ ×©×•×§:</b> {a['market_cap']}{dist_52w_str}\n"
            line += f"ðŸ“ˆ <b>×¢×•×¦×ž×” (RS):</b> {a['rs_65']*100:.1f}% | ðŸ“Š <b>×•×•×œ×™×•×:</b> {a['vol_ratio']:.1f}x\n"
            line += (
                f"ðŸ“ <b>×›×™×•×•×¥ (Tightness):</b> {a['tightness']*100:.1f}% | "
                f"ðŸ”‹ <b>×¢×•×¦×ž×ª ×¡×’×™×¨×”:</b> {a['close_strength']*100:.0f}%\n"
            )
            line += f"ðŸŽ¯ <b>×¤×™×‘×•×˜:</b> ${a['pivot']:.2f} | ðŸ’µ <b>×ž×—×™×¨:</b> ${a['close']:.2f}\n"
            line += f"ðŸ›¡ï¸ <b>×¡×˜×•×¤ ×œ×•×¡:</b> ${a['stop_loss']:.2f} (×¡×™×›×•×Ÿ: {a['risk_pct']:.1f}%-)\n"
            line += f"ðŸ”— <a href='{tv_link}'>×’×¨×£ ×‘-TradingView</a>\n"
            line += "â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€\n"
            return line

        if final_bo:
            msg += f"ðŸ”¥ <b>×¤×¨×™×¦×•×ª ××§×˜×™×‘×™×•×ª ({len(final_bo)}):</b>\n\n"
            for a in final_bo:
                msg += format_stock_block(a, "ðŸš€")
                ...
                save_to_smart_memory(
                    ...
                    a['rs_65'], a['tightness'], a['type'], "Sent"
                )

        if final_wl:
            msg += f"ðŸ‘€ <b>×ž×ª×‘×©×œ×•×ª ×œ×ž×¢×§×‘ ({len(final_wl)}):</b>\n\n"
            for a in final_wl:
                base_line = format_stock_block(a, "â³")
                # ×ž×•×¡×™×£ ×©×•×¨×ª ×ž×¨×—×§ ×œ×¤×™×‘×•×˜ ×¨×§ ×œwatchlist
                base_line = base_line.replace(
                    f"ðŸŽ¯ <b>×¤×™×‘×•×˜:</b> ${a['pivot']:.2f}",
                    f"ðŸŽ¯ <b>×¤×™×‘×•×˜:</b> ${a['pivot']:.2f} (×ž×¨×—×§: {a['dist_to_pivot']:.1f}%)"
                )
                msg += base_line
                log_signal(a['ticker'], a['close'])
                save_to_smart_memory(
        ...
        print("ðŸ’¤ ×”×¡×¨×™×§×” ×”×¡×ª×™×™×ž×”. ×œ× × ×ž×¦××• ×ž× ×™×•×ª ×—×“×©×•×ª ×œ×©×œ×™×—×” ×‘×¡×™×‘×•×‘ ×–×”.")
        send_telegram(
            f"âœ… ×”×¡×¨×™×§×” ×”×¡×ª×™×™×ž×”.\n\n{market_warning}"
            f"××™×Ÿ ×¤×¨×™×¦×•×ª ××• ×ž× ×™×•×ª ×—×“×©×•×ª ×‘×ž×¢×§×‘ ×©×œ× × ×©×œ×—×• ×‘-5 ×”×™×ž×™× ×”××—×¨×•× ×™×."
        )
